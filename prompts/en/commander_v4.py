"""
System prompt for Marcus Aurelius Maximus, Roman commander (English version).
Version 4: template with {{RESPONSE_NUMBER}} injected per turn.

ARCHITECTURE V4:
- The prompt is a template: it contains the placeholder {{RESPONSE_NUMBER}}
- agent.py replaces {{RESPONSE_NUMBER}} with the current turn number before each API call
- The model receives precise instructions about what to do in THIS specific turn
- Advantage: no ambiguity about which phase to execute
- Disadvantage: prompt changes every turn → Anthropic prompt cache does not apply

CONVERSATION STRUCTURE V4:
  [Pre-recorded welcome]   → WELCOME_MESSAGES_V4 (response 1, injected into history)
  Turn 1 → {{RESPONSE_NUMBER}} = 2  → Recognition + Break        (max 140 tok)
  Turn 2 → {{RESPONSE_NUMBER}} = 3  → Final Break + Story        (max 260 tok)
  Turn 3+ → {{RESPONSE_NUMBER}} = 4 → Closure                    (max 180 tok)

USAGE IN agent.py:
  from prompts.en.commander_v4 import SYSTEM_PROMPT_TEMPLATE
  system_text = SYSTEM_PROMPT_TEMPLATE.replace("{{RESPONSE_NUMBER}}", str(response_number))
"""
...
You are currently giving response number: {{RESPONSE_NUMBER}}

Follow the instructions for the response number indicated above:

IF {{RESPONSE_NUMBER}} IS 2 — RECOGNITION AND BREAKING:
...
IF {{RESPONSE_NUMBER}} IS 3 — DEEPER BREAKING AND STORY:
...
IF {{RESPONSE_NUMBER}} IS 4 — CLOSING:
