"""
System prompt per Marco Aurelio Maximus — variante PROVA con prompt caching.

Non sostituisce comandante_v4.py (che resta in uso in produzione) — è un
esperimento separato per testare se il prompt caching Anthropic riduce
tempo/costo senza rompere la coerenza delle risposte.

DIFFERENZA rispetto a comandante_v4.py:
- comandante_v4.py: un'unica stringa con {{NUMERO_RISPOSTA}} sostituito con
  .replace() in 4 punti diversi del testo → il prompt cambia sempre,
  cache Anthropic non si applica mai.
- comandante_v4_cache.py: il testo è spezzato in due blocchi STATICI
  (mai cambiano, cache_control applicato) più un numero di risposta
  passato come blocco separato piccolo, non cachato. I tre blocchi
  "SE LA RISPOSTA È 2/3/4" restano scritti per esteso e identici ad ogni
  turno (prima erano gli unici punti dove serviva sostituire il
  placeholder, ma la sostituzione lì era ridondante: i tre blocchi sono
  comunque sempre tutti presenti, cambia solo quale il modello deve
  seguire, indicato dal numero nel blocco dinamico in mezzo).

USO (nel test, non ancora in agent.py):
...
SE LA RISPOSTA RICHIESTA È 2 — RICONOSCIMENTO E ROTTURA:
...
SE LA RISPOSTA RICHIESTA È 3 — ROTTURA FINALE E RACCONTO:
...
SE LA RISPOSTA RICHIESTA È 4 — CHIUSURA:
